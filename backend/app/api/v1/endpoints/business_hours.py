...
from app.schemas.business_hours import BusinessHoursRead, BulkBusinessHoursUpdate
...
async def get_global_opening_range(
    day_of_week: int = Query(..., ge=0, le=6), db: Session = Depends(get_db)
):
    """
    ...
    merged_ranges.append(
        {"start": current_start.strftime("%H:%M"), "end": current_end.strftime("%H:%M")}
    )
...
async def get_business_hours(
    ...
    business_hours = (
        ...
        .filter(BusinessHours.collaborator_id == collaborator_id)
...
async def bulk_update_business_hours(
    payload: BulkBusinessHoursUpdate, db: Session = Depends(get_db)
):
    """
...
async def delete_business_hours(business_hours_id: int, db: Session = Depends(get_db)):
    ...
    db_bh = (
        db.query(BusinessHours).filter(BusinessHours.id == business_hours_id).first()
    )
    if not db_bh:
